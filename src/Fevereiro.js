...
export default class Fevereiro extends Component
{
...
firebase.auth().onAuthStateChanged((user)=>{
    if(user) {
    firebase.database().ref('historico').child(user.uid).orderByChild('meumes').equalTo('Fevereiro').on('value', (snapshot)=>{
